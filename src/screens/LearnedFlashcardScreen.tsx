...
import {
  View, Text, TextInput, FlatList, Alert, Pressable, ActivityIndicator, StyleSheet,
} from "react-native";
...
import { getLearnedFlashcards, updateWalletFlashcardStatus } from "../api/wallet";
...
const LearnedFlashcardsScreen = () => {
  ...
  useEffect(() => { fetchLearned(); }, [fetchLearned]);

  const moveBackToMainDeck = useCallback(async (id: number) => {
    try {
            ...
            <View style={[styles.miniActionsRow, { justifyContent: "center" }]}>
              <View style={S.btnWrap}>
                <Pressable
                  ...
                  hitSlop={10}
                  style={({ pressed }) => [S.btnPressArea, pressed && S.btnPressAreaPressed]}
                >
                  {({ pressed }) => (
                    <>
  ...
  }, (prev, next) => prev.item.id === next.item.id && prev.item.word === next.item.word && prev.onUnwallet === next.onUnwallet);
