...
const NewFlashcardScreen = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerTitleStyle: { fontFamily: "ArchitectsDaughter", fontSize: 32, color: "#246396" },
      headerRight: () => (
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post(`/api/topics/${selectedTopicId}/flashcards`, { word, definition, example });
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <LinearGradient colors={["#b0f4c9ff", "#313bae8c"]} style={styles.container}>
        <ScrollView
          ...
          keyboardShouldPersistTaps="handled"   // 👈 let taps pass through while keyboard is open
        >
          ...
          <View style={[styles.inputBase, defFocused && styles.inputFocused, { minHeight: 150 }]}>
            <TextInput
              ...
              blurOnSubmit                      // 👈 for multiline to allow returnKey behavior
              returnKeyType="done"
              onSubmitEditing={Keyboard.dismiss}
            />
          </View>
        ...
        <View style={styles.bottomBar}>
          <Pressable onPress={() => { Keyboard.dismiss(); navigation.navigate("Home"); }} hitSlop={10} style={({ pressed }) => [styles.navItem]}>
            {({ pressed }) => (
              <>
          ...
          <Pressable onPress={handleSave} hitSlop={10} style={({ pressed }) => [styles.navItem]}>
            {({ pressed }) => (
              <>
                <Ionicons name="checkmark-circle" size={35} color="#97d0feff" style={pressed && styles.iconGlyphGlow} />
